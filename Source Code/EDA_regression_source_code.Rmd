...
```{r}
time_model_base <- system.time({
  model_base <- glm(inddea~ factor(smokstat)+ age + factor(race)+factor(sex)+factor(histatus)+adjinc+factor(ms),
                  family = binomial,
                  data = ds_base)
})

time_model_arrow <- system.time({
  model_full <- glm(inddea~ factor(smokstat)+ age + factor(race)+factor(sex)+factor(histatus)+adjinc+factor(ms_recode),
                  family = binomial,
                  data = ds_mod)
})
