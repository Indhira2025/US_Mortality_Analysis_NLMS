...
To efficiently load and preprocess the NLMS Tobacco-Use dataset (\~493,000 observations and 43 variables), we compared the performance of base R’s read.csv() with the optimized fread() from the data.table package and Arrow/Parquet function.
...
Benchmarking results demonstrated substantial performance differences across data ingestion methods.
...
bench_results <- bench::mark(
  ...
  read_csv = {
    as.data.frame(read.csv("tu_new.csv"))
  },
  
  fread = {
    as.data.frame(data.table::fread("tu_new.csv"))
  },
  
  arrow_parquet = {
    open_dataset("tu_new.parquet") |> 
      collect() |> 
      as.data.frame()
  },
  
  iterations = 3,   # use >1 for more stable results
  check = FALSE
)
...
saveRDS(bench_results, "benchmark.rds")
...
Benchmarking showed Arrow/Parquet reduced runtime from 2.86s to 0.15s (\~95% improvement), decreased memory usage by \~72%, and reduced garbage collection events by \~83% compared to base R.
...
```{r}
# Death stratifiaction by gender
...
ds_gender <- ds %>%
  ...
  mutate (sex = case_when(sex == 1 ~ "Male",
                          ...
                          TRUE ~ "Unknown")) %>%
  group_by(sex) %>%
  summarise(mortality_rate = round(mean(inddea),2)) %>%
  collect()
...
model_fit <- tibble(
  Model = c("Univariate Model", "Smoking Status Model", "Smoking Status + Smoking Duration Model"),
